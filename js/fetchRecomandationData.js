...
async function handleFormSubmit(event) {
  ...
  const query = `${API_URL}${input}.json`;
...
function displayBooks(books) {
  ...
  for (let book of books) {
    ...
    const description = book.subtitle ? book.subtitle : 'No description available.';
    const isbn = book.isbn ? book.isbn[0] : 'Unknown';
    const key = book.key;
    const publishDate = book.publishDate ? book.publishDate : 'Unknown';
    ...
    const bookHtml = `
      <div class="book-card" onclick="openBook('${key}')">
        ...
        <div class="book-details">
          ...
          <p><b style="margin-right: 30px;">Desc: </b>${description}</p>
          <p><b style="margin-right: 25px;">ISBN: </b>${isbn}</p>
          <p><b style="margin-right: 25px;">Published on: </b>${publishDate}</p>
        </div>
      </div>
    `;
